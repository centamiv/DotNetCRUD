...
namespace DotNetCrud.Render
{
    class InsertPage
    {
        ...
        {
            page.Append("<div class=\"row\"><div class=\"col-12\"><h3>New Product</h3></div></div>");
            page.Append("<hr/>");
            ...
            foreach (var item in _fields)
            {
                ...
                {
                    page.Append("<input name=\"" + item + "\" type=\"number\" class=\"form-control\">");
                }
                else if (classFields[item].StartsWith("DateTime"))
                {
                    ...
                    page.Append("<input name=\"" + item + "\" data-toggle=\"datetimepicker\" type=\"text\" class=\"form-control datetimepicker-input\" data-target=\"#datetimepicker-" + item + "\">");
                ...
                {
                    page.Append("<input name=\"" + item + "\" type=\"text\" class=\"form-control\">");
                }

                page.Append("</div>");
            }
            page.Append("<button type=\"submit\" class=\"btn btn-warning\">Insert</button>");
            page.Append(" <a href=\"/home/index\" class=\"btn btn-secondary\">Cancel</a></form></div></div>");
        }

        internal void Execute<T>(Microsoft.AspNetCore.Http.HttpRequest _request, Microsoft.EntityFrameworkCore.DbContext _db, string _idField, T row, List<string> _fields, Dictionary<string, string> classFields) where T : class, new()
        {
            foreach (var item in _fields.Where(i => i != _idField)) // TODO escludo la chiave principale perchè è autoincrementale
            {
